...
export class Buffer {
    ...
    constructor(
        ...
        this.registry = new FinalizationRegistry<GPUBuffer>(
            (buffer: GPUBuffer) => buffer.destroy()
        );
        this.device = device;
        this.length = length;
        this.dims = dims;
    ...
    copyFrom(
        buffers: GPUBuffer[] | GPUBuffer | Buffer,
    ...
    public write(dataList: Float32Array[] | Float32Array) {
        if (dataList instanceof Float32Array) {
            dataList = [dataList];
        }
        if (dataList.length !== this.dims) {
            throw Error(
                `could not write to ${this.buffers[0].label}, dimension mismatch`
            );
        }
